...
import '../quiz_screens/question_map_screen.dart';
import '../quiz_screens/stats_screen.dart';
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  void initState() {
    super.initState();
    print('Email: ${widget.email}'); // Debug email
    _fetchQuestions();
    _startTimer();
  ...
  Future<void> _fetchQuestions() async {
    try {
      final questions = await FirestoreService(FirebaseFirestore.instance).getQuestions(
        widget.category.id,
        widget.difficulty, // Use difficulty string as difficultyId
        widget.quiz.id,
      );
  ...
  Future<void> _submitQuiz({bool autoSubmit = false}) async {
    ...
    final stats = UserStats(
      userId: widget.email, // Use email as userId
      name: widget.name,
      email: widget.email,
    ...
    print('Saving stats: ${stats.toFirestore()}');
    ...
    try {
      ...
      if (mounted) {
        ...
        // Proceed to StatsScreen even if Firestore fails to show local stats
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (_) => StatsScreen(stats: stats),
